...
const MCP_SERVER_NAME = process.env.WORKIQ_MCP_SERVER_NAME ?? "mcp_CalendarTools";
...
export const workiqCalendarProxy: RequestHandler = async (req, res) => {
  try {
    ...
    console.error("[workiq-calendar-proxy] error:", err);
    if (!res.headersSent) {
      res.status(502).json({
        error: "WorkIQ Calendar proxy failed",
